...
<div class="loadDiv">
<span class="loadImgContainer"><img class="Togepi" src="assets/img/load/load1.gif" alt="Carregando..." onclick="playCrySpecial('https://pokemoncries.com/cries-old/175.mp3')"></span>
...
async function loadPage(offset, limit){
    ...
    pokeAPI.getPokemons(offset, limit).then(
        ...
        listaDePokes.innerHTML = pokemonList.map((pokemon) => `
            <li class="pokemon ${pokemon.type}" id="pokemonBox" value="${pokemon.id}" onclick="showPokemonDexDetails(${pokemon.id})">
                        ...
                        <div class="detail">
                            ...
                            <img src="${pokemon.sprite}" class="pokeImg" alt="${pokemon.name}">
                        </div>
...
async function showPokemonDexDetails(value){
    ...
    pokeAPI.getIndividualDex(value)
    .then((pokemon) => {
        ...
        PokemonIndividual.abilities = pokemon.abilities.map((abilitiesSlot) => (abilitiesSlot.ability.name))
        PokemonIndividual.abilitiesHidden = pokemon.abilities.map((abilitiesStatus) => (abilitiesStatus.is_hidden))
        PokemonIndividual.stats = pokemon.stats.map((statsSlot) => (statsSlot.base_stat))
    ...
    .then(() => { wholeModal.innerHTML = 
        `
<div class="modal-content">
<div class="modal-header ${convertUpperCaseFirstLetter(PokemonIndividual.maintype)}">
<span class="close" onclick="closeModal()">&times;</span>
<h2 class="modalTitle capitalize">${PokemonIndividual.namedex}</h2>
</div>
<div class="modal-body bg-properties ${convertUpperCaseFirstLetter(PokemonIndividual.maintype)}-bg" id="modalDexContent">
<img src="${PokemonIndividual.sprite}" class="pokeImg pokeImgModal" alt="${PokemonIndividual.namedex}" onclick="playCry('${PokemonIndividual.cry}')">
<div class="pokeArrows">
<input type="button" class="pokeArrowButton ArrowLeft ${convertUpperCaseFirstLetter(PokemonIndividual.maintype)}" id="btnPrevPoke" value="<" onclick="showPokemonDexDetails(${(PokemonIndividual.id)-1})">
<input type="button" class="pokeArrowButton ArrowRight ${convertUpperCaseFirstLetter(PokemonIndividual.maintype)}" id="btnNextPoke" value=">" onclick="showPokemonDexDetails(${(PokemonIndividual.id)+1})">
</div>
<div class="infosContainer">
        <div class="dexInfosBody">
       <p><b>Altura:</b> <span class="ApiInfo">${(PokemonIndividual.height)}m</span> / <span class="ApiInfo">${((PokemonIndividual.height)*100).toFixed(0)}cm</span></p>
       <p><b>Peso:</b> <span class="ApiInfo">${PokemonIndividual.weight}kg</span></p>
       <p><b>Habilidades:</b> <span class="ApiInfo">${(PokemonIndividual.abilities).join(', ')}</span></p>
       </div>
       </div>
       </div>
<div class="modal-footer ${convertUpperCaseFirstLetter(PokemonIndividual.secondarytype)}">
<h3 class="modalTitle" id="footerText">Clique no ${convertUpperCaseFirstLetter(PokemonIndividual.namedex)} para ouvir seu som!</h3>
<audio id="pokeCry" src="${PokemonIndividual.cry}"></audio>
    ...
    .then(async()=> {await timeoutFunction(300)})
    .then(() => {playCry(`${PokemonIndividual.cry}`)})
}
}
